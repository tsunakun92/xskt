@props([
    'id' => 'reject-cancel-modal',
    'title' => __('crm::crud.crm-bookings.reject_cancel') ?? 'Reject cancel',
    'notes' => null,
])
    ...
    <div class="relative w-full max-w-md max-h-full mx-auto mt-[15vh]">
        <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
                <form method="POST">
                    @csrf
                    <input type="hidden" name="_method" value="POST" data-reject-method>

                    <div class="space-y-4">
                        <p class="text-sm text-gray-700 dark:text-gray-200">
                            {{ __('crm::crud.reject_confirm_message') }}
                        </p>

                        <div>
                            <label for="{{ $id }}-reject-reason"
                                class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
                                {{ __('crm::crud.crm-bookings.reject_reason') }} <span class="text-red-600">*</span>
                            </label>
                            <textarea id="{{ $id }}-reject-reason" name="reject_reason" rows="3" maxlength="1000" required
                                class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-red-500 focus:ring-red-500 dark:border-gray-500 dark:bg-gray-600 dark:text-white"
                                placeholder="">{{ old('reject_reason') }}</textarea>
                        </div>
    ...
    <script>
        (function() {
            const init = () => {
                ...
                const methodEl = modal.querySelector('[data-reject-method]');
                const reasonEl = modal.querySelector('[name="reject_reason"]');
