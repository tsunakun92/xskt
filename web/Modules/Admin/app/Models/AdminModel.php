...
/**
 * Base model for Admin module.
 * Extends BaseModel and overrides translation methods for admin::crud namespace.
 */
class AdminModel extends BaseModel {
    ...
    //-----------------------------------------------------
    /**
     * Get datatable table columns with Admin translation namespace.
     *
    ...
    public static function getBaseDatatableTableColumns(bool $addActionColumn = true, ?string $keyLang = null): array {
        ...
        foreach ($columns as $column) {
            // Automatically generates the translation key for admin module
            $columnLabels[$column] = __('admin::crud.' . ($keyLang ?? $tableName) . '.' . $column);
        }

        // Add the common 'action' column if needed
        if ($addActionColumn) {
            $columnLabels['action'] = __('admin::crud.action');
        }
    ...
    /**
     * Get fields configuration for form or filter for Admin module.
     *
     ...
     */
    protected static function getAdminFields(string $type, string $routeName, ?string $action = null): array {
        $fields            = [];
        ...
        foreach ($attributesSource as $field) {
            ...
            $fields[$field] = array_merge($defaultAttributes, [
                'label'       => __('admin::crud.' . $routeName . '.' . ($type === self::TYPE_FIELD_FORM ? $field : 'filter.' . $field)),
                'placeholder' => __('admin::crud.' . $routeName . '.' . ($type === self::TYPE_FIELD_FORM ? $field : 'filter.' . $field)),
            ]);
    ...
    /**
     * Get form fields for Admin module.
     *
    ...
    public static function getFormFields(string $routeName, ?string $action = null): array {
        return self::getAdminFields(self::TYPE_FIELD_FORM, $routeName, $action);
    }

    /**
     * Get filter fields for Admin module.
     *
     * @param  string  $routeName
     * @return array<string, array<string, mixed>>
     */
    public static function getFilterFields(string $routeName): array {
        return self::getAdminFields(self::TYPE_FIELD_FILTER, $routeName);
    }
}
