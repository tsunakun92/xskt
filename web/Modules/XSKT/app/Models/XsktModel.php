...
/**
 * Base model for XSKT module.
 * Extends BaseModel directly and overrides translation methods for xskt::crud namespace.
 */
class XsktModel extends BaseModel {
    //-----------------------------------------------------
    // Constants
    //-----------------------------------------------------
    ...
    //-----------------------------------------------------
    /**
     * Get datatable table columns with XSKT translation namespace.
     *
    ...
    public static function getBaseDatatableTableColumns(bool $addActionColumn = true, ?string $keyLang = null): array {
        ...
        foreach ($columns as $column) {
            // Automatically generates the translation key for xskt module
            $columnLabels[$column] = __('xskt::crud.' . ($keyLang ?? $tableName) . '.' . $column);
        }

        // Add the common 'action' column if needed
        if ($addActionColumn) {
            $columnLabels['action'] = __('xskt::crud.action');
        }
    ...
    /**
     * Get fields configuration for form or filter for XSKT module.
     *
     ...
     */
    protected static function getXsktFields(string $type, string $routeName, ?string $action = null): array {
        $fields            = [];
        ...
        foreach ($attributesSource as $field) {
            ...
            $fields[$field] = array_merge($defaultAttributes, [
                'label'       => __('xskt::crud.' . $routeName . '.' . ($type === self::TYPE_FIELD_FORM ? $field : 'filter.' . $field)),
                'placeholder' => __('xskt::crud.' . $routeName . '.' . ($type === self::TYPE_FIELD_FORM ? $field : 'filter.' . $field)),
            ]);
    ...
    /**
     * Get form fields for XSKT module.
     *
    ...
    public static function getFormFields(string $routeName, ?string $action = null): array {
        return self::getXsktFields(self::TYPE_FIELD_FORM, $routeName, $action);
    }

    /**
     * Get filter fields for XSKT module.
     *
     * @param  string  $routeName
     * @return array<string, array<string, mixed>>
     */
    public static function getFilterFields(string $routeName): array {
        return self::getXsktFields(self::TYPE_FIELD_FILTER, $routeName);
    }
}
